...
void AudioCallback(void *userdata, Uint8 *stream, int length) {
	...
	ModuleVideo *video = (ModuleVideo*)userdata;
...
{
	ModuleVideo* player = (ModuleVideo*)param;
	if (player->playing)
		player->refresh = true;
...
bool ModuleVideo::PostUpdate(float dt)
{
	if (playing)
	{
...
int ModuleVideo::PlayVideo(std::string file_path)
{
	if (playing)
	{
...
void ModuleVideo::OpenStreamComponent(int stream_index)
{
	...
	case AVMEDIA_TYPE_VIDEO:
		...
		uint dst_w, dst_h;
		App->window->GetWindowSize(dst_w, dst_h);
...
void ModuleVideo::DecodeVideo()
{
	...
	if (!playing)
		return;
	if (video.pktqueue.GetPacket(&pkt, quit) < 0) //I hope you didn't look at this before decoding the audio...
	{
		if (quit)
			video.finished = true;
		else
	...
	// Uncomment this after TODO 5 for some cool debug on the window title.
...
int ModuleVideo::DecodeAudio()
{
	AVPacket pkt;
	int ret;
	int len2, data_size = -1;
	...
	// UNCOMMENT this for TODO 3
	// Convert the frame to a sample format that SDL can use
	...
	//Put the converted data into audio_buf

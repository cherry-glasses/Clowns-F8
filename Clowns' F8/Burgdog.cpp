...
void Burgdog::Attack(const std::vector<std::pair<int, int>> *_path)
{
	...
	if (_path->size() == 2) {
		...
		if (current_animation->isDone()) {
			...
			App->entity_manager->ThrowAttack(objective_position, current_stats.AtkF, ENTITY_TYPE::ENTITY_ENEMY_BURGDOG);
...
void Burgdog::Ability_1(const std::vector<std::pair<int, int>> *_path)
{
	...
	if (_path->size() == 2) {
		...
		if (current_animation->isDone()) {
			current_animation->Reset();
			timer_skill_1 = 0;
			App->entity_manager->ThrowAttack(objective_position, current_stats.AtkF*1.5, ENTITY_TYPE::ENTITY_ENEMY_BURGDOG);
